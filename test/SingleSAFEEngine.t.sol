...
import {SAFEEngineRewards} from '../src/SAFEEngineRewards.sol';
import {LiquidationEngine} from 'geb/single/LiquidationEngine.sol';
import {AccountingEngine} from 'geb/single/AccountingEngine.sol';
import {TaxCollector} from 'geb/single/TaxCollector.sol';
import 'geb/shared/BasicTokenAdapters.sol';

import {EnglishCollateralAuctionHouse} from 'geb/test/single/SingleCollateralAuctionHouse.t.sol';
import {DebtAuctionHouse} from 'geb/test/single/SingleDebtAuctionHouse.t.sol';
import {PostSettlementSurplusAuctionHouse} from 'geb/test/single/SingleSurplusAuctionHouse.t.sol';
...
contract TestSAFEEngine is SAFEEngineRewards {
...
contract Usr {
    SAFEEngineRewards public safeEngine;
    constructor(SAFEEngineRewards safeEngine_) public {
...
contract SingleAccumulateRatesTest is DSTest {
    ...
    function setUp() public {
        safeEngine = new SAFEEngineRewards();
        safeEngine.initializeCollateralType("gold");
...
contract DebtRewardsMock {
    address public lastAddress;
    uint256 public lastDebt;
    bool public forceRevert;

    constructor (bool revert_) public {
        forceRevert = revert_;
    }

    function setDebt(address who, uint256 val) external {
        require(!forceRevert, "forceRevert");
        lastAddress = who;
        lastDebt = val;
    } 
}

contract UnexistentRewardsTest is SingleModifySAFECollateralizationTest {
    function setUp() public override {
        super.setUp();
        safeEngine.modifyParameters("rewards", address(0xb0b0));
    }
}

contract RevertingRewardsTest is SingleModifySAFECollateralizationTest {
    DebtRewardsMock public rewards;
    function setUp() public override {
        super.setUp();
        rewards = new DebtRewardsMock(true);
        safeEngine.modifyParameters("rewards", address(rewards)); 
    }
}

contract NonRevertingRewardsTest is SingleModifySAFECollateralizationTest {
    DebtRewardsMock public rewards;
    function setUp() public override {
        super.setUp();
        rewards = new DebtRewardsMock(false);
        safeEngine.modifyParameters("rewards", address(rewards)); 
    }
}